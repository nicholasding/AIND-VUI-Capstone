...
def cnn_rnn_model(input_dim, filters, kernel_size, conv_stride,
    ...
    # TODO: Add batch normalization
    bn_rnn = BatchNormalization(name='simple_rnn')(simp_rnn)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    first_rnn = BatchNormalization(name='bn_rnn_1')(
        GRU(units, activation='relu',
            return_sequences=True, implementation=2, name='simp_rnn_1')(input_data)
    )
    last_rnn_layer = first_rnn
    for i in range(1, recur_layers):
        rnn_layer = BatchNormalization(name='bn_rnn_%d' % (i + 1))(
            GRU(units, activation='relu',
                return_sequences=True, implementation=2, name='simp_rnn_%d' % (i + 1))(last_rnn_layer)
        )
        last_rnn_layer = rnn_layer
...
def cnn_bidirectional_rnn_model(input_dim, units, output_dim=29):
    """ Build a bidirectional recurrent network for speech
    """
    # Main acoustic input
    input_data = Input(name='the_input', shape=(None, input_dim))
